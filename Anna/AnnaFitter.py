...
class AnnaFitter:
    ...
    def CheckBinning(self, binning):
        ...
        if type(binning[0]) == list():
            try:
                ...
                error(
                    " --- Wrong bin format ({}), should be list()"
                    .format(type(binning[1])))
                return False
        ...
        if is2D is False:
            ...
            try:
                ...
                error(
                    " Not enought bins ({}), required at least 2 "
                    .format(len(binning[0][1:])))
                ok = False
            try:
    ...
    def CreateBackgroundPDF(self, range):
        ...
        try:
            ...
            error(
                'Cannot find {} in Ostap.FitModels for background function'
    ...
    def DecodeFitType(self, fit_type):
        ...
        for entry in fit_type:
            key, value = self.GetKeyValue(entry, '=')
            try:
            ...
            try:
                ...
                continue
            if self._fit_key[key] is None:
                self._fit_key[key] = value
            else:
                warning(
                    " Oups ! Multiple entries for {}, will take the first one !"
                    .format(key))

        if self._fit_key['signal'] is None or self._fit_key['bkgr'] is None:
            error(
                " Expecting 1 entries with \'signal\' and \'bkgr\',"
                " but found none ...")
            return False
